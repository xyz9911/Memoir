...
class GSAR2RNavBatch(object):
    ...
    def __init__(
        ...
        for d in self.data:
            data_dict[d['scan']].append(d)
            self.env_traj_num[d['scan']] += 1
        if 'train' not in name and 'aug' not in name:
            agg_data = []
            for scan in data_dict.keys():
                self.data = data_dict[scan]
                self.seed = seed
        ...
        self._build_tour_data(self.data)
        ...
        if 'train' in name or 'aug' in name:
            self.seed = seed
            random.seed(self.seed)
    ...
    def _build_tour_data(self, data):
        self.tour_data = {}
        for item in data:
            if item['scan'] not in self.tour_data:
                self.tour_data[item['scan']] = [[item['instr_id']]]
            else:
                self.tour_data[item['scan']][0].append(item['instr_id'])
    ...
    def _next_minibatch(self, is_last, batch_size=None, **kwargs):
        ...
        if self.tour_batch is None or (all(is_last) and self.ix >= len(self.full_data)):
            if "train" not in self.name and "aug" not in self.name:
                self.reset_epoch(False)
            else:
                self.reset_epoch(True)
            self.tour_batch = self.full_data[:self.batch_size]
            self.ix = 0
            self.ix += batch_size
            if len(self.tour_batch) < batch_size:
                self.tour_batch += [[None] * self.num_robots] * (batch_size - len(self.tour_batch))
            self.tour_batch_probe = np.array([[0 for _ in range(self.num_robots)] for _ in range(batch_size)])
            for i, batch_cluster in enumerate(self.tour_batch):
                for j, item in enumerate(batch_cluster):
                    if item is not None:
                        self.tour_ended[i][j] = False

        self.batch = []
        for i in range(len(self.tour_batch)):
            cluster_batch_tmp = []

            if (None not in self.tour_batch[i]) and all(self.tour_ended[i]) and self.ix < len(self.full_data):
                self.tour_batch_probe[i] = 0
                self.tour_batch[i] = self.full_data[self.ix]
                self.ix += 1
                self.tour_ended[i] = False

            for j, tour in enumerate(self.tour_batch[i]):
                if tour is not None and not self.tour_ended[i][j]:
                    cluster_batch_tmp.append(self.data[tour[self.tour_batch_probe[i][j]]])
                    self.tour_batch_probe[i][j] += 1
                else:
