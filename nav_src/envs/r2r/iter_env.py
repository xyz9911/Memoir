...
class IR2RNavBatch(object):
    ...
    def __init__(
        ...
        self.full_data = self.scale_up(self.data, self.tour_data)
        self.ix = 0
        self.tour_ended = np.ones((batch_size, num_robots), dtype=bool)
    ...
    def scale_up(self, data, tour_data):
        self.data = {ex["instr_id"]: ex for ex in data}
        self.data_to_tour_id = {}
        full_data = []
        for scan, tours in tour_data.items():
            for j, tour in enumerate(tours):
                n = len(tour)
                k = min(n, self.num_robots)
                data_tmp = [tour[i * n // k:(i + 1) * n // k] for i in range(k)]
                for t in data_tmp[0]:
                    self.data_to_tour_id[t] = f"{scan}_{j}"
                full_data.append(data_tmp)
        return full_data
    ...
    def check_last(self):
        if self.tour_batch is None:  # start of env
            ...
            last = True
            for i, batch_cluster in enumerate(self.batch):
                for j, item in enumerate(batch_cluster):
                    if item is not None and item['instr_id'] != self.tour_batch[i][j][-1]:
                        last = False
                    else:
                        self.tour_ended[i][j] = True
            return last

    def check_reach(self):
        reach = True
        batch_tmp = copy.deepcopy(self.batch)
        self.batch = [[None for item in cluster_batch_tmp] for cluster_batch_tmp in batch_tmp]
        for i, cluster_batch_tmp in enumerate(batch_tmp):
            for j, item in enumerate(cluster_batch_tmp):
                if item is not None:
                    state = self.env.sims[i][j].getState()[0]
                    if item['path'][-1] != state.location.viewpointId:
                        self.batch[i][j] = {
                            'scan': state.scanId,
                            'instr_id': item['instr_id'],
                            'heading': state.heading,
                            'path': self.shortest_paths[state.scanId][state.location.viewpointId][item['path'][-1]][:-1]
                        }
                        reach = False
                    else:
                        self.batch[i][j] = {
    ...
    def _next_minibatch(self, is_last, batch_size=None, **kwargs):
        """
        ...
        if is_last:
            batch = self.full_data[self.ix: self.ix + batch_size]
            if len(batch) < batch_size:
                self.reset_epoch(False)
                self.ix = batch_size - len(batch)
                batch += self.full_data[:self.ix]
            else:
                self.ix += batch_size
            self.tour_batch = batch
            self.tour_batch_probe = np.array([[0 for _ in range(self.num_robots)] for _ in range(batch_size)])
            for i, batch_cluster in enumerate(batch):
                for j, item in enumerate(batch_cluster):
                    self.tour_ended[i][j] = False
            self.extra_obs = None
        else:
            self.tour_batch_probe += 1
            batch_tmp = []
            for i, cluster_tour_batch in enumerate(self.tour_batch):
                cluster_batch_tmp = []
                for j, tour in enumerate(cluster_tour_batch):
                    if not self.tour_ended[i][j]:
                        cluster_batch_tmp.append(self.data[tour[self.tour_batch_probe[i][j]]])
                    else:
                        cluster_batch_tmp.append(None)
                batch_tmp.append(cluster_batch_tmp)
            for i, cluster_batch_tmp in enumerate(batch_tmp):
                for j, item in enumerate(cluster_batch_tmp):
                    if item is not None:
                        state = self.env.sims[i][j].getState()[0]
                        if state.location.viewpointId != item['path'][0]:
                            batch_tmp[i][j] = {
                                'scan': state.scanId,
                                'instr_id': item['instr_id'],
                                'heading': state.heading,
                                'path': self.shortest_paths[state.scanId][state.location.viewpointId][
                                            batch_tmp[i][j]['path'][0]][:-1]
                            }
            self.batch = batch_tmp
            self.extra_obs = self._get_path_obs()
        ...
        for i, cluster_tour_batch in enumerate(self.tour_batch):
            ...
            for j, tour in enumerate(cluster_tour_batch):
                if not self.tour_ended[i][j]:
                    ...
                    cluster_batch_tmp.append(None)
            self.batch.append(cluster_batch_tmp)

    def reset_epoch(self, shuffle=False):
        ''' Reset the data index to beginning of epoch. Primarily for testing.
            You must still call reset() for a new episode. '''
        self.tour_ended = np.ones((self.batch_size, self.num_robots), dtype=bool)
        if shuffle:
            if self.num_robots == 1:
                random.shuffle(self.full_data)
            else:
                full_data = []
                for scan, tours in self.tour_data.items():
                    for tour in tours:
                        n = len(tour)
                        k = min(n, self.num_robots)
                        data_tmp = [tour[i * n // k:(i + 1) * n // k] for i in range(k)]
            ...
            for j, item in enumerate(cluster):
                ...
                for j, viewpoint_id in enumerate(item['path']):
                    if not view_id:
                        ...
                        self.sim.newEpisode([item['scan']], [viewpoint_id], [heading], [0])
                    else:
                        heading = (view_id % 12) * math.radians(30)
                        elevation = (view_id // 12 - 1) * math.radians(30)
                        self.sim.newEpisode([item['scan']], [viewpoint_id], [heading], [elevation])
                    state = self.sim.getState()[0]
                    view_id = state.viewIndex
                    feats = self.env.feat_db.get_image_feature(item['scan'], viewpoint_id)
                    candidate = self.make_candidate(feats, item['scan'], viewpoint_id, view_id)
                    feats = np.concatenate((feats, self.angle_feature[view_id]), -1)
                    ob = {
                        'scan': state.scanId,
                        'tour_id': self.data_to_tour_id[item['instr_id']],
                ...
                cluster_obs.append(observations)
    ...
    def reset(self, **kwargs):
        ...
        is_last = self.check_last()
        self._next_minibatch(is_last, **kwargs)
        viewpointIds = []
        headings = []
        scanIds = []
        for i, cluster in enumerate(self.batch):
        ...
        return self._get_obs(t=0), is_last
