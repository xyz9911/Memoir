...
class BaseAgent(object):
    ...
    def get_results(self, detailed_output=False):
        ...
        for k, v in self.results.items():
            output.append({'instr_id': k, 'scan': v['scan'], 'imagined_obs': v['imagined_obs'], 'obs_labels': v['obs_labels'], 'obs_soft_labels': v['obs_soft_labels'], 'imagined_actions': v['imagined_actions'], 'action_labels': v['action_labels'],
                           'trajectory': v['path'], 'gt_path': v['gt_path'], 'gt_length': v['gt_length']})
            if detailed_output:
    ...
    def test(self, iters=None, extended_memory=True, **kwargs):
        ...
        if iters is not None:
            ...
            for i in range(iters):
                for traj in self.rollout(extended_memory=extended_memory, **kwargs):
                    # self.loss = 0
            ...
            full_traj_tmp = {}
            # Do a full round
            jdx_length = len(self.env.full_data)
            prev_ix = self.env.ix
            while True:
                visited = set()
                for traj in self.rollout(extended_memory=extended_memory, **kwargs):
                    ...
                    cluster_id = traj.pop('cluster_id')
                    if cluster_id not in full_traj_tmp:
                        full_traj_tmp[cluster_id] = []
                    if cluster_id not in visited:
                        full_traj_tmp[cluster_id].append([])
                        visited.add(cluster_id)
                    full_traj_tmp[cluster_id][-1].append(traj)

                if self.env.ix != prev_ix:
                    print_progress(self.env.ix, jdx_length, prefix='Validation Progress:', suffix='Complete', bar_length=50)
                    prev_ix = self.env.ix

                if all(self.env.check_last()):
                    # self.history = None
                    # self.history_raw = None
                    # self.history_raw_length = None
                    self.full_results.extend(full_traj_tmp.values())
                    full_traj_tmp = {}
                    if self.env.ix >= jdx_length:
                        for instr_id in self.env.data.keys():
                            if instr_id not in self.results:
                                raise Exception
                        print("looped", looped)
                        break

        # with open(os.path.join(self.args.log_dir, f'{self.env.name}_obs_retrieve_analysis.json'), 'w') as f:
        #     json.dump(self.obs_retrieve_analysis, f)
...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, args, env, rank=0):
        ...
        self.optimizers = self.vln_bert_optimizer
    ...
    def test(self, use_dropout=False, feedback='argmax', extended_memory=True, iters=None):
        ''' Evaluate once on each instruction in the current environment '''
        scan_gmap_bank_bk = self.scan_gmap_bank
        scan_cnt_bk = self.scan_cnt
        self.scan_gmap_bank = {}
        ...
        super().test(iters=iters, extended_memory=extended_memory)

        self.scan_gmap_bank = scan_gmap_bank_bk
        self.scan_cnt = scan_cnt_bk

    def train(self, n_iters, feedback='teacher', extended_memory=True, **kwargs):
        ...
        for iter in range(1, n_iters + 1):
            ...
            if self.args.train_alg == 'imitation':
                ...
                self.rollout(
                    train_ml=1., extended_memory=extended_memory, **kwargs
                )
            elif self.args.train_alg == 'dagger':
                if self.args.ml_weight != 0:
                    self.feedback = 'teacher'
                    self.rollout(
                        train_ml=self.args.ml_weight, extended_memory=extended_memory, **kwargs
                    )
                self.feedback = 'expl_sample' if self.args.expl_sample else 'sample'
                self.rollout(train_ml=1, extended_memory=extended_memory, **kwargs)
            else:
                if self.args.ml_weight != 0:
                    self.feedback = 'teacher'
                    self.rollout(
                ...
                self.rollout(train_ml=None, extended_memory=extended_memory, **kwargs)
