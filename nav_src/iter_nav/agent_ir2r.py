...
from nav_src.iter_nav.agent_base import Seq2SeqAgent
...
class GMapNavAgent(Seq2SeqAgent):
    ...
    def _nav_gmap_variable(self, obs, gmap_id_map, robot_id_map):
        # [stop] + gmap_vpids
        batch_size = len(obs)
        ...
        for i, ob in enumerate(obs):
            ...
            if self.args.forest:
                connected_nodes, other_nodes = self.gmaps[gmap_id].get_connected_nodes(robot_id, full_graph=True)
                local_graph_nodes = self.gmaps[gmap_id].get_connected_nodes(robot_id)
                visited_vpids += other_nodes
                for k in connected_nodes:
                    if k in local_graph_nodes:
                        if self.gmaps[gmap_id].visited(k, robot_id):
                            visited_vpids.append(k)
                        elif self.gmaps[gmap_id].observed(k, robot_id):
                            if self.gmaps[gmap_id].reachable(k, robot_id):
            ...
            gmap_step_ids = [self.gmaps[gmap_id].node_step_ids[robot_id].get(vp, 0) for vp in gmap_vpids]
            if len(gmap_vpids) != 1:
                gmap_img_embeds = [self.gmaps[gmap_id].get_node_embed(vp, robot_id, self.args.forest) for vp in gmap_vpids[1:]]
            ...
            gmap_pos_fts = self.gmaps[gmap_id].get_pos_fts(
            ...
            for u in range(1, len(gmap_vpids)):
                for v in range(u + 1, len(gmap_vpids)):
                    gmap_pair_dists[u, v] = gmap_pair_dists[v, u] = \
                        self.gmaps[gmap_id].distance(gmap_vpids[u], gmap_vpids[v], robot_id)
    ...
    def update_emap_nodes(self, emap_nodes, exp_pairs, exp_sims):
        for emap_node_set, exp_pair, exp_sim in zip(emap_nodes, exp_pairs, exp_sims):
            for pair, sim in zip(exp_pair, exp_sim):
                for j, (node, fts) in enumerate(pair):
                    if fts is not None:
                        if node not in emap_node_set:
                            emap_node_set[node] = [[fts[0], sim[j]]]
                        else:
                            emap_node_set[node].append([fts[0], sim[j]])
        return emap_nodes

    def _nav_emap_variable(self, obs, state_fts, emap_nodes, gmap_id_map, robot_id_map, multimodal_history):
        # [stop] + gmap_vpids
        batch_size = len(obs)
        ...
        for i, nodes in enumerate(batch_nodes):
            ...
            if len(nodes):
                if multimodal_history:
                    batch_emap_embeds[i] = [self.gmaps[gmap_id].get_node_embed(node, robot_id, self.args.forest) + memory_ft for node, memory_ft in zip(nodes, memory_fts[cnt:cnt+len(nodes)])]
                else:
                    batch_emap_embeds[i] = [self.gmaps[gmap_id].get_node_embed(node, robot_id, self.args.forest) for node in nodes]
                cnt += len(nodes)
        ...
        for i, ob in enumerate(obs):
            ...
            if self.args.forest:
                ...
                visited_vpids += other_nodes
                for k in connected_nodes:
                    if self.gmaps[gmap_id].visited(k, robot_id) or k not in batch_nodes[i]:
                        visited_vpids.append(k)
            else:
                local_graph_nodes = self.gmaps[gmap_id].get_connected_nodes(robot_id)
                for k in local_graph_nodes:
            ...
            for node, emap_embed in zip(batch_nodes[i], batch_emap_embeds[i]):
                if node not in visited_vpids and node != ob['viewpoint']:
                    ...
                    if self.gmaps[gmap_id].reachable(node, robot_id):
            ...
            gmap_step_ids = [self.gmaps[gmap_id].node_step_ids[robot_id].get(vp, 0) for vp in gmap_vpids]
            gmap_img_embeds = [self.gmaps[gmap_id].get_node_embed(vp, robot_id, self.args.forest) for vp in visited_vpids]
    ...
    def _nav_vp_variable(self, obs, pano_embeds, cand_vpids, view_lens, nav_types, gmap_id_map, robot_id_map):
        batch_size = len(obs)
        ...
        for i, ob in enumerate(obs):
            gmap_id = gmap_id_map[i]
            robot_id = robot_id_map[i]
            cur_cand_pos_fts = self.gmaps[gmap_id].get_pos_fts(
                ob['viewpoint'], cand_vpids[i],
                ob['heading'], ob['elevation'], robot_id
            )
            cur_start_pos_fts = self.gmaps[gmap_id].get_pos_fts(
                ob['viewpoint'], [self.gmaps[gmap_id].start_vps[robot_id]],
                ob['heading'], ob['elevation'], robot_id
            )
    ...
    def _teacher_path(self, obs, gmap_id_map, robot_id_map, ended):
        """
    ...
    def make_equiv_action(self, a_t, raw_obs, t, robot_id_map, traj):
        """
                ...
                if ob is not None:
                    ...
                    if action is not None:  # None is the <stop> action
                        traj[cnt]['path'].append(
                            self.gmaps[tot_cnt].graph_pool.path(ob['viewpoint'], action))
    ...
    def _oracle_phase(self, tour_not_ended):
        if self.env.extra_obs is not None:
            ...
            cnt = 0
            tot_cnt = 0
            for extra_obs_cluster in self.env.extra_obs:
                for j, extra_ob in enumerate(extra_obs_cluster):
                    if extra_ob is not None:
                        for ob in extra_ob:
                            ...
                            cnt += 1
                    tot_cnt += 1
            self._update_scanvp_cands(extra_obs)

            pano_inputs = self._panorama_feature_variable(extra_obs)
            pano_embeds, pano_masks = self.vln_bert('panorama', pano_inputs)
            avg_pano_embeds = torch.sum(pano_embeds * pano_masks.unsqueeze(2), 1) / \
                              torch.sum(pano_masks, 1, keepdim=True).detach()
            pano_values = self.vln_bert('transform', {"pano_embeds": avg_pano_embeds})
            for i, ob in enumerate(extra_obs):
                gmap_id = gmap_id_map[i]
                i_vp = extra_obs[i]['viewpoint']
                self.gmaps[gmap_id].update_graph(ob, robot_id_map[i])
                self.gmaps[gmap_id].update_node_embed(i_vp, avg_pano_embeds[i], robot_id_map[i], rewrite=True,
                                                      value=pano_values[i])
                for j, (i_cand_vp, i_view_id) in enumerate(
                        zip(pano_inputs['cand_vpids'][i], pano_inputs['view_ids'][i])):
                    if not self.gmaps[gmap_id].observed(i_cand_vp, robot_id_map[i]):
                        self.gmaps[gmap_id].update_node_embed(i_cand_vp, pano_embeds[i, j], robot_id_map[i],
                                                              rewrite=False, view_id=i_view_id)
    ...
    def rollout(self, train_ml=None, reset=True, extended_memory=True):
        ...
        if extended_memory:
            self._oracle_phase(tour_not_ended)
        ...
        if tour_start:
            ...
            for i, cluster_obs in enumerate(raw_obs):
                ...
                self.gmaps.extend([gmap] * len(cluster_obs))
        else:
            cnt = 0
            for i, cluster_obs in enumerate(raw_obs):
                start_vps = [None if ob is None else ob['viewpoint'] for ob in cluster_obs]
                self.gmaps[cnt].reset(start_vps, clear=not extended_memory)
                cnt += len(cluster_obs)
        ...
        for t in range(self.args.max_action_len):
            for i, ob in enumerate(obs):
                gmap_id = gmap_id_map[i]
                if not ended[i]:
                    self.gmaps[gmap_id].node_step_ids[robot_id_map[i]][ob['viewpoint']] = t + 1
            ...
            for i, ob in enumerate(obs):
                ...
                if not ended[i]:
                    ...
                    for j, (i_cand_vp, i_view_id) in enumerate(
                        ...
                        if not self.gmaps[gmap_id].observed(i_cand_vp, robot_id_map[i]):
                            self.gmaps[gmap_id].update_node_embed(i_cand_vp, pano_embeds[i, j], robot_id_map[i],
                                                                  rewrite=False, view_id=i_view_id, redundant_view=self.args.redundant_view)
                ...
                if t > 0:
                    ...
                    for i in range(batch_size):
                        if true_valid_flags[i]:
                            ...
                            # gt_gsnn_img_embeds_batch[0][i].append(avg_pano_embeds[i].detach())
                    ...
                    # kl
                    kl_losses += torch.distributions.kl_divergence(transit_outputs['posterior_dist'], transit_outputs['prior_dist']).mean(dim=-1) * true_valid_flags
            ...
            with torch.inference_mode():
                ...
                teacher_paths, teacher_stop_flags = self._teacher_path(obs, gmap_id_map, robot_id_map, ended)
                for i, ob in enumerate(obs):
                    gmap = self.gmaps[gmap_id_map[i]]
                    if not ended[i]:
                        if not stop_flag[i]:
                            ...
                            for j, step_nodes in enumerate(beam_nodes):
                                new_nodes = [node for node in step_nodes if not self.gmaps[gmap_id_map[i]].retrieved(node, robot_id_map[i])]
                ...
                emap_nodes = self.update_emap_nodes(emap_nodes, exp_pairs, exp_sims)

            experience_inputs, emap_teacher_masks = self._nav_emap_variable(obs, state_fts, emap_nodes, gmap_id_map, robot_id_map, self.args.multimodal_history)
            ...
            # navigation policy
            nav_inputs, gmap_teacher_masks = self._nav_gmap_variable(obs, gmap_id_map, robot_id_map)
            nav_inputs.update(
                self._nav_vp_variable(
                    obs, pano_embeds, pano_inputs['cand_vpids'],
            ...
            for i, ob in enumerate(obs):
                ...
                if not ended[i]:
                    i_vp = obs[i]['viewpoint']
                    self.gmaps[gmap_id].node_stop_scores[robot_id_map[i]][i_vp] = {
            ...
            raw_obs = self.make_equiv_action(cpu_a_t, raw_obs, t, robot_id_map, traj)
            ...
            for i in range(batch_size):
                ...
                if (not ended[i]) and just_ended[i]:
                    ...
                    for k, v in self.gmaps[gmap_id].node_stop_scores[robot_id_map[i]].items():
                        if v['stop'] > stop_score['stop']:
                            stop_score = v
                            stop_node = k
                    if stop_node is not None and obs[i]['viewpoint'] != stop_node:
                        traj[i]['path'].append(
                            self.gmaps[gmap_id].graph_pool.path(obs[i]['viewpoint'], stop_node))
            ...
            for i, ob in enumerate(obs):
                ...
                gmap_id = gmap_id_map[i]
                if not ended[i]:
                    target_path = self.gmaps[gmap_id].graph[robot_id].path(last_vps[i], target_names[i])
                    pred_path = self.gmaps[gmap_id].graph[robot_id].path(last_vps[i], ob['viewpoint'])
            ...
            for i, ob in enumerate(obs):
                if not ended[i]:
                    gmap_id = gmap_id_map[i]
                    robot_id = robot_id_map[i]
                    aux_path = self.gmaps[gmap_id].path(last_vps[i], ob['viewpoint'], robot_id)[:-1]
                    if t > 0:
                        self.gmaps[gmap_id].add_experience(last_vps[i], ob['instr_id'], [state_fts[i].detach(), pred_obs_multisteps[i].detach(), recon_outs['img_embed'][i].detach()], pred_masks[i], aux_path)
                    else:
                        self.gmaps[gmap_id].add_experience(last_vps[i], ob['instr_id'], None, None, aux_path)
        ...
        full_gt_rewards = torch.FloatTensor(full_gt_rewards).to(txt_embeds.device) if len(
            full_gt_rewards) > 0 else None
        ...
        for i in range(batch_size):
            ...
            for part in traj[i]['path']:
                for node in part:
                    neg_set.discard(node)
            all_neg_img_embeds_batch[i].extend([self.gmaps[gmap_id_map[i]].node_embeds_pool[vp][0] for vp in neg_set])
            all_neg_img_embeds.extend([self.gmaps[gmap_id_map[i]].node_embeds_pool[vp][0] for vp in neg_set])
            assert sum([self.gmaps[gmap_id_map[i]].node_embeds_pool[vp][1] for vp in neg_set]) == 0
        ...
        # oracle phase-2
        with torch.inference_mode():
            if extended_memory and (not self.env.check_reach()):
                self._oracle_phase(tour_not_ended)
