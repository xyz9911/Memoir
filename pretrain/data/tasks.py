...
class MlmDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        txt_ids, txt_labels = random_word(inputs['instr_encoding'], self.vocab_range, self.mask_token_id)
        ...
        output['gmap_node_types'] = torch.LongTensor(inputs['gmap_node_types'])
...
def mlm_collate(inputs):
    ...
    batch['gmap_node_types'] = pad_sequence(batch['gmap_node_types'], batch_first=True, padding_value=0)
...
class SapDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inputs = self.nav_db.get_input_sap(idx, end_vp_type, return_act_label=True)
...
def custom_pad_sequence(sequences, max_length, batch_first=True, padding_value=0):
    padded = pad_sequence(sequences, batch_first=batch_first, padding_value=padding_value)

    batch_size = padded.size(0) if batch_first else padded.size(1)
    current_length = padded.size(1) if batch_first else padded.size(0)

    shape = [batch_size, max_length] if batch_first else [max_length, batch_size]
    if len(padded.shape) > 2:
        shape.extend(padded.shape[2:])

    result = torch.full(shape, padding_value, dtype=padded.dtype, device=padded.device)

    copy_length = min(current_length, max_length)
    if batch_first:
        result[:, :copy_length] = padded[:, :copy_length]
    else:
        result[:copy_length] = padded[:copy_length]
...
class SrDataset(Dataset):
    ...
    def __getitem__(self, idx):
        input = self.nav_db.get_input_sr(idx)
        output = {}
        output['txt_ids'] = torch.LongTensor(input['instr_encoding'])
        output['scans'] = input['scan']
        output['headings'] = input['heading']
        output['paths'] = input['path']
        output['gt_paths'] = input['gt_path']
        # output['neg_paths'] = input['neg_path']
        return output

def sr_collate(inputs):
    batch = {
        k: [x[k] for x in inputs] for k in inputs[0].keys()
    }
    # text batches
    batch['txt_lens'] = torch.LongTensor([len(x) for x in batch['txt_ids']])
    batch['txt_ids'] = pad_sequence(batch['txt_ids'], batch_first=True, padding_value=0)
    # batch['txt_ids'] = custom_pad_sequence(batch['txt_ids'], max_length=80, batch_first=True, padding_value=0)
    return batch
