...
from .ops import extend_neg_masks, gen_seq_masks, pad_tensors_wgrad, zdistr
...
class GlobalMapEncoder(nn.Module):
    ...
    def gmap_input_embedding(
            self, split_traj_embeds, split_traj_vp_lens, traj_vpids, traj_cand_vpids, gmap_vpids,
            gmap_step_ids, gmap_node_types, gmap_pos_fts, gmap_lens
    ):
        ...
        gmap_embeds = gmap_img_fts + \
                      ...
                      self.node_type_embeddings(gmap_node_types)
    ...
    def forward(
            ...
            split_traj_embeds, split_traj_vp_lens, traj_vpids, traj_cand_vpids, gmap_vpids,
            gmap_step_ids, gmap_node_types, gmap_pos_fts, gmap_lens, graph_sprels=None
    ):
...
class RSSMPriorBlock(nn.Module):
    ...
    def forward(self, stoch_act, deter):
        s = self.input_stoch_act(stoch_act)
        s = self._norm(s)
...
class GlocalTextPathCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.belief_head = nn.Sequential(nn.Linear(config.hidden_size + 14, config.hidden_size), nn.ReLU(), nn.Linear(config.hidden_size, config.hidden_size))
        self.state_fusion = nn.Sequential(nn.Linear(config.hidden_size * 2, config.hidden_size), nn.ReLU(), nn.Linear(config.hidden_size, config.hidden_size))
        self.world_model_type = config.world_model_type
        self.recurrent = config.recurrent
        
        if self.world_model_type.lower() == 'crssm':
            # RSSM
            self.stoch_size = 32
            self.stoch_discrete = 32
            self.deter_size = 7168
            self.bottleneck_size = 1024
            self.lang_init = Project(config.hidden_size, self.deter_size, self.deter_size)
        ...
        elif self.world_model_type.lower() == 'rssm':
            # RSSM
            self.stoch_size = 96
    ...
    def forward(
        ...
        if return_gmap_embeds:
            gmap_embeds = self.global_encoder(
                ...
                split_traj_embeds, split_traj_vp_lens, traj_vpids, traj_cand_vpids, gmap_vpids,
                gmap_step_ids, gmap_node_types, gmap_pos_fts, gmap_lens, graph_sprels=gmap_pair_dists,
            )
    ...
    def forward_navigation(
        ...
        if self.recurrent:
            gmap_embeds = torch.cat((txt_embeds[:, 0:1, :], gmap_embeds), dim=1)
            gmap_embeds = self.global_encoder.encoder(
                txt_embeds[:, 1:, :], txt_masks, gmap_embeds, gmap_masks,
                graph_sprels=graph_sprels
            )
            global_state = gmap_embeds[:, 0, :]
            gmap_embeds = gmap_embeds[:, 1:, :]
        else:
            gmap_embeds = self.global_encoder.encoder(
                txt_embeds, txt_masks, gmap_embeds, gmap_masks,
                graph_sprels=graph_sprels
        ...
        if self.recurrent:
            ...
            vp_embeds = torch.cat((txt_embeds[:, 0:1, :], vp_embeds), dim=1)
            vp_embeds = self.local_encoder.encoder(txt_embeds[:, 1:, :], txt_masks, vp_embeds, vp_masks)
            local_state = vp_embeds[:, 0, :]
            vp_embeds = vp_embeds[:, 1:, :]
            state_embeds = global_state + local_state
        else:
            vp_embeds = vp_img_embeds + self.local_encoder.vp_pos_embeddings(vp_pos_fts)
        ...
        if self.sap_fuse_linear is None:
            ...
            if self.recurrent:
                fuse_weights = torch.sigmoid(self.sap_fuse_linear(
                    torch.cat([global_state, local_state], 1)
                ))
            else:
                fuse_weights = torch.sigmoid(self.sap_fuse_linear(
    ...
    def forward_transit(self, prev_states, obs_embed, encoder_embeds, encoder_attention_mask, past_key_values=None):
        ...
        if 'rssm' in self.world_model_type:
            prior_mu_std, prior_deter = self.prior(prev_states[1], prev_states[0])
    ...
    def forward_imagine(self, cur_states, encoder_embeds, encoder_attention_mask, past_key_values=None):
        if 'rssm' in self.world_model_type:
            batch_size = cur_states[1].size(0)
            prior_mu_std, cur_deter = self.prior(cur_states[1], cur_states[0])
        elif 'tssm' in self.world_model_type:
